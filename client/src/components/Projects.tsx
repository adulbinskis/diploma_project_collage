...
const Projects: FC =()=> {
    ...
    const filteredDuplicate = projects.filter(projects => {
        // check if the current brand is a duplicate
        const isDuplicate: boolean = seen.has(projects.year);
        // add the current brand to the Set
        seen.add(projects.year);
        // filter() returns the brand when isDuplicate is false
        return !isDuplicate;
    });

    const filteredYear = projects.filter(projects => {
        return projects.year === selectedYear;
    });
    ...
    async function deleteArchive(id: string){
        if(selectedYear)
        await ProjectService.deleteProject(id, selectedYear, selectedTitle)
        await getProjects()
    }

    async function handleSave(){
       await ProjectService.editProjectText(editedText, editedID)
        setIsEditText(false)
        await getProjects()
    }
    ...
    async function deleteImage(picture: string, archiveId: string){
        ...
        await ProjectService.deleteProjectPicture(picture, archiveId)
        await getProjects()
    }
    ...
    async function handleSaveToServer(year: number, title: string){
        ...
        await ProjectService.saveImage(data, year, title)
        await getProjects()
    }
    ...
    async function changeActuality(id: string, actuality: boolean){
        await ProjectService.changeActuality(id, actuality)
        await getProjects()
    }
    ...
    return(
        <div className='archive-container'>
            <h1 className="archive-title">Projekti </h1>
